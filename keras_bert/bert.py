...
def get_model(token_num,
    ...
    if training:
        ...
        # rssi_dense_layer = LayerNormalization(
        #     name='rssi-norm',
        # )(transformed)
        # # rssi_pred_layer = EmbeddingSimilarity(name='rssi-Sim')([rssi_dense_layer, embed_weights])
        # rssi_pred_layer = keras.layers.Dense(
        #     units=1,
        #     activation="relu",
        #     name='rssi-pred',
        # )(rssi_dense_layer)
        # rssi_masked_layer = Masked(name='rssi')([rssi_pred_layer, inputs[-1]])
...
def batch_generator(token_dict,
                    token_list,
                    input_label_list,
                    output_label_list,
                    batch_size=128):
    ...
    :param token_dict:numpy.ndarray
    :param token_list:list<list>
    :param mask_rate:
    ...
    :return:
    """
    outputs = output_label_list
    outputs = np.concatenate((outputs, token_list))  ## 为了完整截取bachsize
    data_size = len(token_list)
    token_inputs,  label_inputs = [], []
    i = 0
    while True:
        token_input, label_input = token_list[i], input_label_list[i]
        # has_mask = False
        # tokens = token_list[i]
        # # rssis = rssi_list[i]
        # for j in range(len(tokens)):
        #     if mask_rate and np.random.random() < mask_rate:
        #         has_mask = True
        #         masked_input.append(1)
        #         r = np.random.random()
        #         if r < mask_mask_rate:
        #             token_input.append(token_dict[TOKEN_MASK])
        #             # rssi_input.append(token_dict[TOKEN_MASK])
        #         elif r < mask_mask_rate + mask_random_rate:
        #             token_input.append(np.random.randint(token_start_idx, token_start_idx+26*60))
        #             # rssi_input.append(np.random.randint(20, 90))
        #         else:
        #             token_input.append(tokens[j])
        #             # rssi_input.append(rssis[j])
        #     else:
        ...
        now_size = len(token_inputs)
        i = (i+1) % data_size
        if now_size >= batch_size:
            inputs = [np.asarray(x) for x in [token_inputs, label_inputs]]
            upidx = i + data_size
            outputs = np.asarray(np.expand_dims(outputs[upidx-now_size:upidx], axis=-1))
            token_inputs,  label_inputs = [], []
...
#     :return:
#     """
#     mlm_outputs = token_list[:][:]
#     # rssi_outputs = rssi_list[:][:]
#     mlm_outputs = np.concatenate((mlm_outputs, token_list))
#     # rssi_outputs = np.concatenate((rssi_outputs, rssi_list))
#     data_size = len(token_list)
#     token_start_idx = len(get_base_dict())
#     # Generate MLM
#     token_inputs,  masked_inputs = [], []
#     i = 0
#     while True:
#         token_input, masked_input = [], []
#         has_mask = False
#         tokens = token_list[i]
#         # rssis = rssi_list[i]
#         for j in range(len(tokens)):
...
#             upidx = i + data_size
#             outputs = np.asarray(np.expand_dims(mlm_outputs[upidx-now_size:upidx], axis=-1))
#             token_inputs,  masked_inputs = [], []
#             yield inputs, outputs


def gen_batch_inputs(rssi_list,
                     token_dict,
                     token_list,
                     mask_rate=0.15,
                     mask_mask_rate=0.8,
    ...
    token_start_idx = len(get_base_dict())
    # Generate MLM
    token_inputs, rssi_inputs, masked_inputs = [], [], []
    for i in range(batch_size):
        token_input, masked_input, rssi_input = [], [], []
        has_mask = False
        tokens = token_list[i]
        rssis = rssi_list[i]
        for j in range(len(tokens)):
            if mask_rate and np.random.random() < mask_rate:
                ...
                if r < mask_mask_rate:
                    ...
                    token_input.append(np.random.randint(token_start_idx, len(token_dict)))
                    rssi_input.append(np.random.randint(20, 90))
                else:
                    token_input.append(tokens[j])
                    rssi_input.append(rssis[j])
        ...
        masked_inputs.append(masked_input)
        rssi_inputs.append(rssi_input)
    inputs = [np.asarray(x) for x in [token_inputs, rssi_inputs, masked_inputs]]
    outputs = [np.asarray(np.expand_dims(x, axis=-1)) for x in [mlm_outputs, rssi_outputs]]
